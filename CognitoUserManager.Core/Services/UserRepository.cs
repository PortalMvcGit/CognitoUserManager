    ...
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<AuthResponseModel> TryLoginAsync(UserLoginModel model)
        {
            try
            {
                ...
                var result = await AuthenticateUserAsync(model.UserName, model.Password);
            ...
            {
                var listUsersResponse = await FindUsersByEmailAddress(model.UserName);

                if (listUsersResponse != null && listUsersResponse.HttpStatusCode == HttpStatusCode.OK)
                {
                    var users = listUsersResponse.Users;
                    var filtered_user = users.FirstOrDefault(x => x.Attributes.Any(x => x.Name == "email" && x.Value == model.UserName));
        ...
        private async Task<Tuple<CognitoUser, AuthenticationResultType>> AuthenticateUserAsync(string username, string password)
        {
            CognitoUser user = new CognitoUser(username, _cloudConfig.AppClientId, _userPool, _provider);
